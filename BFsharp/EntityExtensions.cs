...
namespace BFsharp
{
    public class EntityExtensions : IEntityExtensions, INotifyPropertyChanged
    {
        ...
        protected void SafeRecursiveActionInternal(Action<EntityExtensions, int> action, Dictionary<object, object> visited, int level, ClusterTraversalMode mode)
        {
            ...
            if (mode == ClusterTraversalMode.SelfOnly) return;
        ...
        private void SafeRecursiveActionInternal(Action<EntityExtensions> action)
        {
            var visited = new Dictionary<object, object>(ObjectReferenceEqualityComparerer<object>.Default);
            SafeRecursiveActionInternal((e,n)=>action(e), visited, 0, ClusterTraversalMode.All);
        }

        public void DoClusterAction(Action<IEntityExtensions> action)
        {
            DoClusterAction((e, n) => action(e), ClusterTraversalMode.All);
        }
        ...
        public void DoClusterAction(Action<IEntityExtensions, int> action)
        {
            DoClusterAction(action, ClusterTraversalMode.All);
        }
        ...
        {
            SafeRecursiveActionInternal(ex => { ex.IsDirty = false; });
        }
        ...
        public bool IsDirty
        {
            ...
            {
                bool isDirty = false;
                SafeRecursiveActionInternal(ex => { isDirty |= ex._isDirty; });
    ...
    public class EntityExtensions<T> : IEntityExtensions<T>, INotifyPropertyChanged, IGetInternalEntityExtensions
    {
        ...
        public void DoClusterAction(Action<IEntityExtensions, int> action, ClusterTraversalMode mode)
        {
            _extensions.DoClusterAction(action, mode);
        }

        public void DoClusterAction(Action<IEntityExtensions, int> action)
        {
            _extensions.DoClusterAction(action);
        }
